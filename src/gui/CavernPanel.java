...
public class CavernPanel extends JPanel {
    ...
    public CavernPanel(Cavern cav, int screenWidth, int screenHeight, GUI gui) {
        ...
        addMouseListener(new MouseAdapter() {
            ...
            public void mousePressed(MouseEvent e) {
                var row= e.getY() / TILE_HEIGHT;
                var col= e.getX() / TILE_WIDTH;
    ...
    public void setLighting(boolean light) {
        for (var i= 0; i < cavern.getRowCount(); i++ ) {
            for (var j= 0; j < cavern.getColumnCount(); j++ ) {
                visited[i][j]= light;
            }
        }
        repaint();
    ...
    public BufferedImage getGoldIcon(Node n) {
        ...
        var spriteIndex= (int) gold;
        var rowIndex= spriteIndex / COIN_SPRITES_PER_ROW;
        var colIndex= spriteIndex % COIN_SPRITES_PER_ROW;
    ...
    public void paintComponent(Graphics page) {
        ...
        for (var i= 0; i < getWidth(); i+= 100) {
            page.drawImage(background, i, 0, 100, getHeight(), null);
        }
        page.setColor(darkness);

        // Draw the maze tiles
        for (var row= 0; row < cavern.getRowCount(); row++ ) {
            for (var col= 0; col < cavern.getColumnCount(); col++ ) {
