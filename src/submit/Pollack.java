...
public class Pollack extends Finder {
     ...
     * @return The method returns a true if the orb is found, false otherwise.
     * This recursive helper function runs depth-first-search on a sorted array of neighbors.
     */
    private boolean findHelper(FindState state) {
        var initial= state.currentLoc();
        var neigs= new ArrayList<NodeStatus>();
        for (NodeStatus neighbor : state.neighbors()) {
            neigs.add(neighbor);
        }
        Collections.sort(neigs);
        for (NodeStatus neighbor : neigs) {
            long id= neighbor.getId();
            var ndist= neighbor.getDistanceToTarget();
            if (!visited.contains(id)) {
                ...
                visited.add(id);
                state.moveTo(id);
                if (findHelper(state)) { return true; }
    ...
    public void flee(FleeState state) {
        ...
        Heap<List<Node>> goldies= getAllGoldies(current);
        while (state.currentNode() != state.exit()) {
            optMove(goldies, state);
            goldies= getAllGoldies(state.currentNode());
        }
        return;
    }

    /**
     * > This function returns a heap of all the goldies in the map, sorted by the amount of gold
     * divided by the distance to the goldie from the current node
     * 
     * @param current the current node
     * @return A heap of lists of nodes.
     */
    private Heap<List<Node>> getAllGoldies(Node current) {
        Heap<List<Node>> goldies= new Heap<List<Node>>(false);
        for (Node node : nodes) {
            int gold= node.getTile().gold();
            if (gold > 0) {
                List<Node> path= Path.shortestPath(current, node);
                double priority= gold / path.size();
                goldies.insert(path, priority);
            }
        }
        return goldies;
    }
    ...
    /**
     * If the path to the exit is shorter than the path to the next goldie, then go to the exit.
     * Otherwise, go to the next goldie
     * 
     * @param goldies a heap of lists of nodes, where each list of nodes is a path to a gold node.
     * @param state the current state of the game
     */
    private void optMove(Heap<List<Node>> goldies, FleeState state) {
        if (goldies.size() == 0) {
            moveAlong(state, Path.shortestPath(state.currentNode(), state.exit()));
            return;
        }
        List<Node> list= goldies.peek();
        Node toCheck= list.get(list.size() - 1);
        List<Node> pathToExit= Path.shortestPath(toCheck, state.exit());
        if (Path.pathSum(pathToExit) <= state.stepsLeft() - Path.pathSum(list)) {
            moveAlong(state, list);
            return;
        } else {
            moveAlong(state, Path.shortestPath(state.currentNode(), state.exit()));
            return;
     ...
     * "Move along the path."
     * 
     * The function takes in a FleeState and a List of Nodes. It iterates over the nodes in the list and moves to the successive nodes
     * 
     * @param state The state of the agent.
     * @param path The path to the target node
     */
    private void moveAlong(FleeState state, List<Node> path) {
        path.remove(0);
        for (Node node : path) {
            state.moveTo(node);
        }
